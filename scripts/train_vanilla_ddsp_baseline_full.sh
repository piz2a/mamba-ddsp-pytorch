...
SINGLE_RUN="vanilla_ddsp_single_note_${STAMP}"
RIFF_RUN="vanilla_ddsp_riff_${STAMP}"

echo "Device: ${DEVICE}"
echo "Batch: ${BATCH}"
echo "Single-note baseline run: ${SINGLE_RUN} (${SINGLE_STEPS} steps)"
echo "Riff baseline run:        ${RIFF_RUN} (${RIFF_STEPS} steps)"
...
python -m bass_ddsp.train \
  --config configs/vanilla_ddsp_baseline_single_note.yaml \
  --name "${SINGLE_RUN}" \
  --root runs \
  --steps "${SINGLE_STEPS}" \
  --batch "${BATCH}" \
...
python -m bass_ddsp.train \
  --config configs/vanilla_ddsp_baseline_riff.yaml \
  --name "${RIFF_RUN}" \
  --root runs \
  --steps "${RIFF_STEPS}" \
  --batch "${BATCH}" \
...
echo "Done."
echo "Single-note baseline run: /workspace/runs/${SINGLE_RUN}"
echo "Riff baseline run:        /workspace/runs/${RIFF_RUN}"
