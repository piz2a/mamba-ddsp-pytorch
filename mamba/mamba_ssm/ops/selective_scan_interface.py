...
class SelectiveScanFn(torch.autograd.Function):
    ...
    def forward(ctx, u, delta, A, B, C, D=None, z=None, delta_bias=None, delta_softplus=False,
        ...
        if selective_scan_cuda is None:
            raise RuntimeError(
                "selective_scan_cuda is not available. Install/build mamba-ssm "
                "CUDA extensions or call selective_scan_fn with the reference "
                "fallback enabled."
            )
        if u.stride(-1) != 1:
            u = u.contiguous()
...
def rms_norm_forward(
    ...
    if _layer_norm_fwd is None:
        raise RuntimeError("The fused Triton layer norm kernel is not available.")
    # x (b l) d
    if x.stride(-1) != 1:
        x = x.contiguous()
