# implementing decoder-only transformer by myself, without LLM
...
with open('/workspace/handcode-transformer/input.txt', 'r', encoding='utf-8') as f:
    text = f.read()
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, x):
        out = torch.cat([h(x) for h in self.heads], dim=-1)  # (B, T, hs * num_heads = D)
