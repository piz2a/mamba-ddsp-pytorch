...
def extract_pitch(signal, sampling_rate, block_size):
    ...
    try:
        ...
        def _install_torchaudio_stub():
            ...
            def _missing_load(*args, **kwargs):
                raise RuntimeError(
                    "torchaudio is not installed. DDSP preprocessing loads "
                    "audio through librosa, so torchcrepe.predict still works, "
                    "but torchcrepe file-loading helpers are unavailable."
                )
        ...
        raise RuntimeError(
            "extract_pitch requires torchcrepe and its audio dependencies. "
            "Install torchcrepe and a torchaudio build matching your torch "
            "installation before running preprocessing."
        ) from exc
    ...
    f0 = f0_tensor.squeeze(0).detach().cpu().numpy().reshape(-1)
    if f0.shape[-1] == length + 1:
        f0 = f0[:-1]
