...
def pulses_from_gate(gate):
    previous = np.concatenate([[0.0], np.asarray(gate[:-1])])
    gate = np.asarray(gate)
    return ((gate > 0.5) & (previous <= 0.5)).astype(np.float32), ((gate <= 0.5) & (previous > 0.5)).astype(np.float32)
...
def extract_aubio_onsets(y, config):
    ...
    events = {name: np.zeros(frame_count, dtype=np.float32) for name in names}
    ...
    for index in range(frame_count):
        ...
        for name in names:
            ...
            if bool(decision[0]):
                ...
                events[name][reported_index] = 1.0
    return {"times": np.arange(frame_count, dtype=np.float32) * config.hop_seconds,
            ...
            "complex_onset": events["complex"], "hfc_onset": events["hfc"],
...
def extract_voice_controls(audio, config=None, source_name=None):
    ...
    energy_activity = 1.0 / (1.0 + np.exp(-(features["rms_db"] - (np.percentile(features["rms_db"], config.noise_floor_percentile) + config.energy_margin_db)) / config.energy_softness_db))
    gate = causal_hysteresis(energy_activity, 0.5, 0.35)
    activity_onset, _ = pulses_from_gate(gate)
    # No offset detector is claimed in this laboratory yet.
    offset = np.zeros_like(gate)
    note_onset = np.maximum(activity_onset, np.maximum(aubio_complex_event, aubio_hfc_event)).astype(np.float32)
    ...
    control_tensor = np.stack([f0, masked_zscore(features["rms_db"], gate > 0.5), periodicity,
                               onset_strength, note_onset, gate, note_age,
    ...
    control_names = ["f0_hz_torchcrepe", "loudness_rms_z", "periodicity_torchcrepe", "onset_strength_hpss_percussive",
                     "note_onset", "gate_energy_hysteresis", "note_age_seconds", "aubio_complex_novelty",
    ...
    return {**features, "y": y, "sr": config.sample_rate, "source_name": source_name, "config": config,
            ...
            "energy_activity": energy_activity, "activity_evidence": energy_activity, "gate": gate,
...
def plot_voice_control_dashboard(result):
    ...
    axes[2].set_title("aubio onset detection: continuous strength and binary decisions")
