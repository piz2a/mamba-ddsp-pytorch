...
This fork keeps `penn` and `torchcrepe` optional because they require a
`torch`/`torchaudio` build that matches the local CUDA environment. MIDI
rendering does not need them. If they are unavailable during feature
preprocessing, `feature.py` falls back to `librosa.pyin`; set
`DDSP_GUITAR_ALLOW_PITCH_FALLBACK=0` to fail instead.
